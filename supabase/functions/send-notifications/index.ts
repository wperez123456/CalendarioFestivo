...
Deno.serve(async () => {
  ...
  if (weekday === 'Sun' && hour === 15 && minute < 10) jobs.push('weekly-summary');
  ...
  for (const profile of profiles || []) {
    ...
    const relevant = jobs.includes('weekly-summary')
      ? (events || []).filter(e => e.date >= addDays(today, 1) && e.date <= addDays(today, 6)).sort((a, b) => String(a.date).localeCompare(String(b.date)) || compareEventTime(a, b))
    ...
    const body = jobs.includes('weekly-summary')
      ? `Recordatorio: ${relevant.map(e => `${formatDate(e.date)} ${formatEventTime(e)} ${e.title}`).join(' · ')}`
      : `Tienes ${relevant.length} evento(s) hoy.`;
    ...
    if (!error) {
      for (const subscription of (subscriptions || []).filter(s => s.profile_id === profile.id)) {
        try { await webpush.sendNotification({ endpoint: subscription.endpoint, keys: { p256dh: subscription.p256dh, auth: subscription.auth } }, JSON.stringify({ title: jobs.includes('weekly-summary') ? 'Recordatorio' : 'Eventos', body, url: './' })); }
...
function compareEventTime(a: { event_time?: string | null; start_time?: string | null; title?: string }, b: { event_time?: string | null; start_time?: string | null; title?: string }) {
  const aTime = a.event_time || a.start_time || ''; const bTime = b.event_time || b.start_time || '';
  if (!aTime && !bTime) return String(a.title || '').localeCompare(String(b.title || ''), 'es');
  if (!aTime) return 1; if (!bTime) return -1; return aTime.localeCompare(bTime);
}
function formatDate(dateText: string) { return new Intl.DateTimeFormat('es-ES', { weekday: 'short', day: 'numeric', month: 'short', timeZone: 'Europe/Madrid' }).format(new Date(`${dateText}T12:00:00Z`)); }
function formatEventTime(event: { event_time?: string | null; start_time?: string | null; all_day?: boolean }) {
  if (event.all_day) return 'todo el día';
  const value = event.event_time || event.start_time;
